...
<cffunction output="false" name="getUserById" access="public" returntype="struct" hint="Fetch user details by user ID along with related table data">
    ...
    <!--- SQL Query to join all related tables and select necessary fields --->
    <cfquery result="result" name="qUserDetails">
        ...
        FROM
            taousers u
        INNER JOIN 
            dateformats df ON u.dateformatid = df.id
        LEFT JOIN 
            timezones t ON u.tzid = t.tzid
        LEFT JOIN 
            thrivecart tc ON u.customerid = tc.id
        WHERE 
            u.userid = <cfqueryparam value="#arguments.userId#" cfsqltype="cf_sql_integer">
    </cfquery>
    ...
    <cfif qUserDetails.recordCount EQ 1>
        <cfset user = {
            "user": {
                "dateformatID": qUserDetails.dateformatID ?: 1,
                "dateformatExample": qUserDetails.dateformatExample ?: "mm/dd/yyyy",
                "userID": qUserDetails.userID,
                "calendarName": qUserDetails.calendarName,
                "useremail": qUserDetails.userEmail,
                "userfirstName": qUserDetails.CustomerFirst,
                "userlastName": qUserDetails.CustomerLast
            },
            "timezone": {
