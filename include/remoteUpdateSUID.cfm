...
<cfif suid neq "0">
    <!--- Set oldSystemType from reldetails --->
    <cfset oldSystemType = reldetails.systemtype />
<cfelse>
    <!--- Default oldSystemType to "None" --->
...
<form action="/include/systemchange.cfm">
    ...
    <center>
        <div class="radio radio-info form-check-inline">
            <input type="radio" id="N" value="None" name="new_systemtype" <cfif oldSystemType eq "None">checked="checked"</cfif> />
            <label for="N"> None </label>
        </div>
        <div class="radio form-check-inline">
            <input type="radio" id="T" value="Targeted List" name="new_systemtype" <cfif oldSystemType eq "Targeted List">checked="checked"</cfif> />
            <label for="T"> Targeted List </label>
        </div>
        <div class="radio form-check-inline">
            <input type="radio" id="F" value="Follow Up" name="new_systemtype" <cfif oldSystemType eq "Follow Up">checked="checked"</cfif> />
            <label for="F"> Follow Up </label>
        </div>
        <div class="radio form-check-inline">
            <input type="radio" id="M" value="Maintenance List" name="new_systemtype" <cfif oldSystemType eq "Maintenance List">checked="checked"</cfif> />
...
<!--- Modifications: Removed unnecessary cfoutput tags (Standard 2), removed # symbols in conditional checks (Standard 3), standardized variable names and casing (Standard 5), ensured consistent attribute quoting, spacing, and formatting (Standard 6). --->