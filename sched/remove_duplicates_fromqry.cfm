...
<cfloop query="getDuplicateQueries">
    ...
    <cfquery result="result"  name="getFilesForQuery" datasource="abod">
        SELECT id, parents
        FROM tao_files
        WHERE qry_details = <cfquery result="result" param value="#getDuplicateQueries.qry_details#" cfsqltype="cf_sql_longvarchar">
    </cfquery>
    ...
    <cfloop from="2" to="#getFilesForQuery.recordcount#" index="i">
        ...
        <cfquery result="result"  datasource="abod">
            UPDATE tao_files
            SET status = 'deleted'
            WHERE id = <cfquery result="result" param value="#getFilesForQuery.id[i]#" cfsqltype="cf_sql_integer">
        </cfquery>
        ...
        <cfquery result="result"  name="getParentFiles" datasource="abod">
            SELECT id, parents
            FROM tao_files
            WHERE FIND_IN_SET(<cfquery result="result" param value="#getFilesForQuery.id[i]#" cfsqltype="cf_sql_integer">, parents)
        </cfquery>
        ...
        <cfloop query="getParentFiles">
            ...
            <cfif ListFind(getParentFiles.parents, getFilesForQuery.id[i])>
                ...
                <cfquery result="result"  datasource="abod">
                    UPDATE tao_files
                    SET parents = <cfquery result="result" param value="#newParents#" cfsqltype="cf_sql_varchar">
                    WHERE id = <cfquery result="result" param value="#getParentFiles.id#" cfsqltype="cf_sql_integer">
                </cfquery>
            </cfif>
        </cfloop>
    </cfloop>
