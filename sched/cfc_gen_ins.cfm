...
SELECT t.id, f.id as function_id, f.function_name, f.component_id, f.qry_type, 
    ...
    WHERE t.qry_type = 'ins' and (f.function_code is null or f.function_code = '')
</cfquery>
...
<cfloop query="getInsertFunctions">
    ...
    <cfsavecontent variable="generatedPrompt"><cfoutput>
        <html>
            <body>
                <p><strong>Goal:</strong> Generate a ColdFusion function to process an insert query.</p>
                <p>The function should insert values into the table <strong>#getInsertFunctions.qry_table#</strong>.</p>
                ...
                <ul>
                    ...
                    <li>If arguments are not provided in the CFC, those fields should be omitted from the insert operation.</li>
                    <li>Support flexible inserts by conditionally adding fields based on the provided arguments.</li>
                    <li>Do not add a component tag, just the function.</li>
                    <li>
                        If the return type is "numeric", return the primary key value as "rec_id" using 
                        SQL functions like SCOPE_IDENTITY() or LAST_INSERT_ID().
                    </li>
                </ul>
    ...
    <!--- Process the API response: Replace backticks, remove excess parts, and add the correct datasource --->
    <cfif structKeyExists(responseData, "choices")>
        ...
        <!--- Modify to return the primary key if the returntype is "numeric" --->
        <cfif getInsertFunctions.returntype EQ "numeric">
            <cfset generatedFunctionCode &= chr(13) & '<cfreturn SCOPE_IDENTITY() as "rec_id">'>
        </cfif>
