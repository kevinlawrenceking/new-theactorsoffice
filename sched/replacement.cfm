...
<cfloop query="getDeletedRecords">
    ...
    <cfquery result="result"  name="getReplacementRecord" datasource="abod">
        SELECT id
        FROM tao_files
        WHERE qry_details = <cfquery result="result" param value="#getDeletedRecords.qry_details#" cfsqltype="cf_sql_longvarchar">
        AND status <> 'deleted'
        LIMIT 1
    </cfquery>
    ...
    <cfif getReplacementRecord.recordcount GT 0>
        <cfquery result="result"  datasource="abod">
            UPDATE tao_files
            SET replacement_id = <cfquery result="result" param value="#getReplacementRecord.id#" cfsqltype="cf_sql_integer">
            WHERE id = <cfquery result="result" param value="#getDeletedRecords.id#" cfsqltype="cf_sql_integer">
        </cfquery>
    </cfif>
</cfloop>
